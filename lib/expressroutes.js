            ...
            get: function(req, key) {
                if (param.type === 'file' && typeof req['files'] === 'object') {
                    ...
                    if(Array.isArray(req.files)) {
                        for (var upfile of req.files) {
                            if (upfile['fieldname'] === key) {
                                return upfile['originalname'];
                            }
                        }
                    }
                    else if (req.files[key]) {
                        var upfile = req.files[key];
                        return upfile['originalname'];
                    }
                }
                else if (param.type === 'file' && typeof req['file'] === 'object' && req.file['fieldname'] === key) {
                    var upfile = req.file;
                    return upfile['originalname'];
                }
                else {
